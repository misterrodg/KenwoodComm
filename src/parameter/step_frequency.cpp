...
core::Result<void> StepFrequency::setStepFrequency(const std::string& input) {
    ...
    if (std::regex_match(input, match, regex)) {
        ...
        if (frequencyUnit != FrequencyUnit::UNKNOWN &&
            ...
            return core::Error{core::ErrorCode::ValueOutOfRange,
                               "Step frequency must be between 0 and " +
                                   std::to_string(MAX_STEP_FREQUENCY_VALUE) +
                                   "Hz."};
        }

        return core::Error{core::ErrorCode::InvalidFrequencyUnit,
                           "Step frequency unit must be Hz or kHz."};
    }

    return core::Error{core::ErrorCode::InvalidStepFrequency,
                       "Invalid step frequency format: '" + input +
...
std::string StepFrequency::getStepFrequencyString() const {
    char formattedFrequency[6];
    snprintf(formattedFrequency, sizeof(formattedFrequency), "%05d",
             frequencyInHz);
