...
void Serial::Open() {
    ...
    settings.c_oflag &= ~OPOST;  /* Prevent special interpretation of output
    ...
    settings.c_cc[VTIME] =
        1; /* Wait for up to 1 deciseconds (unit of 0.1 seconds), returning as
...
std::string Serial::Read() {
    ...
    bufPtr = buffer;
    while ((readIn = read(fdPort, bufPtr,
